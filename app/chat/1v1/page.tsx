...
function Chat1v1Component() {
        ...
        (payload) => {
          ...
          if (newMessage.sender_role === 'system' && 
            ...
            if (exitingUserName !== currentUserName) {
              console.log('Other participant exited, automatically ending session:', exitingUserName)
  ...
  const handleExitConfirm = async () => {
    ...
    if (roomId) {
      try {
        ...
        console.log('Sending exit notification to room:', roomId)
  ...
  const handleSurveySubmit = async (responses: PostSurveyResponses) => {
    try {
      ...
      console.log("1v1 survey responses:", responses)
      ...
      console.log("1v1 survey saved successfully:", result)
      
      setShowExitSurvey(false)
      setShowSurveyThankYou(true)
    } catch (error) {
      console.error("Error submitting 1v1 survey:", error)
